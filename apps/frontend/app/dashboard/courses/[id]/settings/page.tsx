    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
                ...
                <span>/</span>
                <Link href={`/dashboard/courses/${id}`} className="hover:text-foreground">
                    {title || "Course"}
                </Link>
                <span>/</span>
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                {!editsLocked && autoSave.status === "saving" && (
                    <span className="text-xs text-muted-foreground">Saving...</span>
                )}
                {!editsLocked && autoSave.status === "saved" && (
                    <span className="text-xs text-success">All changes saved</span>
                )}
                {!editsLocked && autoSave.status === "error" && (
