...
function CourseDetailContent() {
    ...
    useEffect(() => {
        if (!token || !id) return;
        let cancelled = false;
        Promise.all([coursesApi.getById(token, id), modulesApi.list(token, id)])
    ...
    const saveModuleOrder = useCallback(async () => {
        ...
        try {
            await coursesApi.reorderModules(
                token,
                id,
                latestModulesRef.current.map(m => m._id),
            );
    ...
    const tabs: Tab[] = isMentor ? ["modules", "members", "snapshots"] : ["modules", "members"];
                ...
                <>
                    ...
                    {modules.length === 0 && !showNewModule ? (
                        <p className="text-muted-foreground text-sm">
                            No modules yet. {isMentor ? "Add one to get started." : ""}
                        </p>
                    ) : (
