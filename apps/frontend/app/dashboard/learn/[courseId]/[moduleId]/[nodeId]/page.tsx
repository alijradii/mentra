    ...
    <div className="flex h-[calc(100vh-64px)] overflow-hidden">
      ...
      <aside
        className={`
          ...
          w-72 shrink-0 bg-white border-r flex flex-col
                    ...
                    return (
                      <li key={n._id}>
                        ...
                        >
                          {done ? (
                            <svg
                              className={`w-3.5 h-3.5 shrink-0 ${active ? "text-green-300" : "text-green-500"}`}
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
                            >
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
          ...
          <button
            ...
            className="lg:hidden mb-4 flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700"
          >
          ...
          <div className="mb-8">
            ...
            {node?.description && (
              <p className="text-gray-500 mt-2 text-sm">{node.description}</p>
            )}
          </div>

          {/* Sections */}
          {(node?.sections?.length ?? 0) === 0 ? (
            <p className="text-gray-400 italic text-sm">This lesson has no content yet.</p>
          ) : (
