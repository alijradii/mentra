          ...
          {quizSections.map((section) => {
            ...
            return (
              <div key={section.id} className="space-y-2">
                ...
                {graded && (
                  <div className={`p-3 rounded-lg text-sm ${graded.isCorrect ? "bg-success/15 text-success border border-success/40" : "bg-destructive/15 text-destructive border border-destructive/40"}`}>
                    <p className="font-semibold">
                      {graded.isCorrect ? "Correct" : "Incorrect"} — {graded.autoScore ?? 0}/{graded.maxScore ?? 0} pts
                    </p>
                  </div>
                )}
              </div>
        ...
        {pastAttempts.length > 1 && (
          <div className="mt-6">
            ...
            <div className="space-y-1">
              {pastAttempts.map((a) => (
                <div key={a._id} className="flex items-center justify-between text-sm px-3 py-2 rounded bg-background border">
                  <span>Attempt #{a.attemptNumber}</span>
                  <span className="font-medium">{a.autoScore ?? 0}/{a.maxScore}</span>
                </div>
              ))}
            </div>
          </div>
...
function InteractiveSequence({ quiz, answer, onAnswer }: { quiz: any; answer: unknown; onAnswer: (v: unknown) => void }) {
...
function InteractiveMatching({ quiz, answer, onAnswer }: { quiz: any; answer: unknown; onAnswer: (v: unknown) => void }) {
...
function InteractiveFillBlank({ quiz, answer, onAnswer }: { quiz: any; answer: unknown; onAnswer: (v: unknown) => void }) {
...
function InteractiveClassification({ quiz, answer, onAnswer }: { quiz: any; answer: unknown; onAnswer: (v: unknown) => void }) {
  const items = quiz.items ?? [];
