        ...
        return (
            <div className="space-y-4">
                ...
                {canRetake && !isDuePassed ? (
                    <Button onClick={handleStart}>{attemptCount === 0 ? "Start quiz" : "Retake quiz"}</Button>
                ) : (
                                ...
                                .map(s => (
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            setPhase(s.status === "released" ? "released" : "submitted");
                                        }}
    ...
    switch (quizType) {
        case "mcq": {
            ...
            const toggle = (optId: string) => {
                if (isMultiple) {
                    onAnswer(
                        selected.includes(optId) ? selected.filter((id: string) => id !== optId) : [...selected, optId],
                    );
                } else {
                    onAnswer([optId]);
                }
