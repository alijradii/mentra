...
export function CourseMembersPanel({ courseId, token, currentUserId, ownerId, isMentor, editsLocked = false }: Props) {
            ...
            <section>
                ...
                {loadingMentors ? (
                    ...
                    <ul className="space-y-2">
                        {mentors.map(m => (
                            <li key={m._id} className="flex items-center gap-3 p-3 bg-card rounded-lg border">
                        ...
                        {mentors.length === 0 && <p className="text-sm text-muted-foreground">No teachers yet.</p>}
                    </ul>
                )}
                    ...
                    <form onSubmit={handleAddMentor} className="mt-4 flex gap-2 items-start">
                        <div className="flex-1">
                            <Input
                                ...
                                onChange={e => {
                                    setAddMentorEmail(e.target.value);
                                    setAddError("");
                                }}
            ...
            <section>
                <h2 className="text-lg font-semibold text-foreground mb-4">
                    ...
                    {!loadingStudents && students.length > 0 && (
                        <span className="ml-2 text-sm font-normal text-muted-foreground">({students.length})</span>
                    )}
                </h2>
                ...
                {loadingStudents ? (
                    ...
                    <ul className="space-y-2">
                        {students.map(s => (
                            <li key={s._id} className="flex items-center gap-3 p-3 bg-card rounded-lg border">
